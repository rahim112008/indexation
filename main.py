...
# --- PAGE REPRODUCTION & ALERTES ---
elif menu == "❤️ Reproduction & Alertes":
    st.title("❤️ Suivi de la Reproduction & Alertes")
    
    col_a, col_b = st.columns(2)
    
    with col_a:
        st.subheader("🍼 Agnelages à venir")
        try:
            if 'saillies_db' in st.session_state and not st.session_state.saillies_db.empty:
                # Filtrer les gestations confirmées
                avenir = st.session_state.saillies_db[st.session_state.saillies_db['Gest_Confirme'] == 'Oui'].copy()
                if not avenir.empty:
                    st.dataframe(avenir[['ID_Brebis', 'Date_Agnelage_Prevu', 'ID_Belier']], hide_index=True)
                else:
                    st.info("Aucun agnelage prévu prochainement.")
        except Exception as e:
            st.error(f"Erreur affichage prévisions : {e}")

    with col_b:
        st.subheader("📊 Statistiques de Reproduction")
        try:
            if 'agnelages_db' in st.session_state and not st.session_state.agnelages_db.empty:
                # Calcul simple du taux de prolificité
                total_agnelages = len(st.session_state.agnelages_db)
                total_vivants = st.session_state.agnelages_db['Nombre_Vivants'].sum()
                prolificite = total_vivants / total_agnelages if total_agnelages > 0 else 0
                
                st.metric("Taux de Prolificité Moyen", f"{prolificite:.2f} agneaux/portée")
                st.write(f"Total agneaux nés : {total_vivants}")
        except Exception as e:
            st.warning("Données statistiques en attente de saisie.")

    st.divider()
    st.subheader("🔔 Historique des Alertes")
    if alerts:
        for a in alerts:
            st.write(f"{a['icon']} **{a['type'].replace('_', ' ').upper()}** - {a.get('id', a.get('brebis', ''))} (Sévérité: {a['severite']})")
    else:
        st.info("Tout est sous contrôle. Aucune alerte enregistrée.")

# ============================================================
# FIN DU PROGRAMME - SÉCURITÉ SYNTAXE
# ============================================================
